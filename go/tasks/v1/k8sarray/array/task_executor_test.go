...
func TestExecutor_StartTask(t *testing.T) {
	...
	status, err := e.StartTask(ctx, newMockTaskContext(types.TaskPhaseUnknown, nil), newTaskTemplate(), nil)
...
func newMockExecutor(t *testing.T, runtimeClient client.Client, memStore *storage.DataStore) Executor {
	...
	if runtimeClient == nil {
		assert.NoError(t, e.InjectClient(mocks.NewMockRuntimeClient()))
	} else {
		assert.NoError(t, e.InjectClient(runtimeClient))
	}

	assert.NoError(t, e.InjectCache(&informertest.FakeInformers{}))

	assert.NoError(t, e.Initialize(context.TODO(), types.ExecutorInitializationParameters{
		EventRecorder: MockEventRecorder{},
...
func TestExecutor_CheckTaskStatus(t *testing.T) {
	t.Run("Happy path", func(t *testing.T) {
		...
		status, err = e.CheckTaskStatus(ctx, newMockTaskContext(status.Phase, status.State), newTaskTemplate())
...
func TestExecutor_KillTask(t *testing.T) {
	e := newMockExecutor(t, nil, nil)
	ctx := context.Background()
	_, err := e.StartTask(ctx, newMockTaskContext(types.TaskPhaseUnknown, nil), newTaskTemplate(), nil)
	assert.NoError(t, err)
	assert.NoError(t, e.KillTask(ctx, newMockTaskContext(types.TaskPhaseUnknown, nil), "test kill"))
}
...
func TestExecutor_ResolveOutputs(t *testing.T) {
	...
	m, err := e.ResolveOutputs(ctx, newMockTaskContext(types.TaskPhaseUnknown, customState), "x", "[1].x")
	assert.NoError(t, err)
