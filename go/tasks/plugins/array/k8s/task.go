...
func (t Task) Launch(ctx context.Context, tCtx core.TaskExecutionContext, kubeClient core.KubeClient) (bool, error) {
	newState := t.State
	podTemplate, _, err := FlyteArrayJobToK8sPodTemplate(ctx, tCtx)
	if err != nil {
		return false, errors2.Wrapf(ErrBuildPodTemplate, err, "Failed to convert task template to a pod template for a task")
	}
	...
	if err != nil {
		return false, errors2.Wrapf(ErrReplaceCmdTemplate, err, "Failed to replace cmd args")
	}
	...
	if err != nil && !k8serrors.IsAlreadyExists(err) {
		...
		return false, errors2.Wrapf(ErrSubmitJob, err, "Failed to submit job.")
	}
