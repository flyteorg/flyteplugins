...
func newClientWithMockBatch() *client {
	...
	return NewCustomBatchClient(mocks.NewMockAwsBatchClient(), "test-region", rateLimiter, rateLimiter).(*client)
}

func TestClient_SubmitJob(t *testing.T) {
	ctx := context.Background()
	rateLimiter := utils.NewRateLimiter("Get", 1000, 1000)
	c := NewCustomBatchClient(mocks.NewMockAwsBatchClient(), "test-region", rateLimiter, rateLimiter).(*client)
